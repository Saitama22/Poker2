...
namespace poker.exe._2._0
{
    ...
    static class Deck
    {
        ...
        {
        //    start = 7;
        //    Card tempCard1 = new Card(7, 'p');
        //    cards[0] = tempCard1;
        //    Card tempCard2 = new Card(2, 'p');
        //    cards[1] = tempCard2;
        //    Card tempCard3 = new Card(4, 'k');
        //    cards[2] = tempCard3;
        //    Card tempCard4 = new Card(3, 'k');
        //    cards[3] = tempCard4;
        //    Card tempCard5 = new Card(8, 'k');
        //    cards[4] = tempCard5;
        //    Card tempCard6 = new Card(12, 'k');
        //    cards[5] = tempCard6;
        //    Card tempCard7 = new Card(4, 'k');
        //    cards[6] = tempCard7;
        }
    ...
    class Player
    {
        ...
        {
            Random rateRnd = new Random();
            int newRand = rateRnd.Next(1, 10);
            if ((sum - rate) < (1000 * newRand)-5000)
                giveCards();
            else if (max != 0 && numbCycl == 0 && motion != 0)
            {
                motion = 3 - motion;
              ...
              //  if (motion==0)
                {
                    ...
                    {
                        int newrate = 100 * (10 - comb);
                        if (newrate > rate)
                            rate = newrate;
                    }
                }
            }            
            //else if (motion==0&&rate==0)
            //{
            //    motion = 3 - motion;
            //    int comb = onlycomb(motion);
            //}
    ...
    class Game
    {
        ...
        {
            (int idComb, int CardPow1, int CardPow2, char suit)[] comb = new (int, int, int, char)[player.Length];
            int numbwin = 0;
            ...
            {
                if (comb[i].idComb < comb[numbwin].idComb)
                    numbwin = i;
                else if (comb[i].idComb == comb[numbwin].idComb)
                {
                    if (comb[i].CardPow1 > comb[numbwin].CardPow1)
                        numbwin = i;
                    else if (comb[i].CardPow1 == comb[numbwin].CardPow1)
                    {
                        if (comb[i].CardPow2 > comb[numbwin].CardPow2)
                            numbwin = i;
                        else if (comb[i].CardPow2 == comb[numbwin].CardPow2)
                        {
                            if (player[i].max() > player[numbwin].max())
                                numbwin = i;
                            else if (player[i].max() == player[numbwin].max())
                                if (player[i].min() > player[numbwin].min())
                                    numbwin = i;
                        }
                    }
                }
            }
            string namecomb = nameComb(comb[numbwin].idComb);
            return (namecomb, comb[numbwin].CardPow1, comb[numbwin].CardPow2, comb[numbwin].suit, numbwin + 1);
        }
